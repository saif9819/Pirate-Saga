...
public class Boards : MonoBehaviour
{
    ...
    private MatchType ColumnOrRow()
    {
        ...
        matchType.type = 0;
        matchType.color = "";
        //Cycle through all of match and decide of a bomb need to be made
    ...
    private void DamageConcrte(int column, int row)
    {
        if (column > 0)
        {
        ...
        while(!slime && loops < 200)
        {
            ...
            if (slimeTiles[newX, newY])
            {
                ...
                if(adjacent != Vector2.zero)
                {
                    ...
                    Vector2 temPosition = new Vector2(newX + (int)adjacent.x, newY + (int)adjacent.y);
                    GameObject tile = Instantiate(slimePiecePrefab, temPosition, Quaternion.identity);
